define(function (require, exports, module) {
                ...
                $filterEl($el, '[sn-binding],[sn-model],[sn-on]').each(function () {
                    ...
                    if (on) {
                        ...
                        on.replace(revents, function (match, event, name, key) {
                            ...
                            actions[event] = getVariable(variable) || variable;
                        });
                        eventid++;
                    }
    ...
    $.extend(ViewModel.prototype, {
        ...
        handleEvent: function (e) {
            ...
            if (events) {
                ...
                if (typeof fn == 'string')
                    ...
                    fn = $(target).closest('[sn-repeat-name="' + fn.collection + '"]')[0].snModel.get(fn.variable);
                }
                fn && fn(e);
            }
        },
        ...
        bindEvents: function () {
            var self = this
            for (var i = 0, len = this.finder.events; i < len; i++) {
                var event = this.finder.events[i]
                if (this.events.indexOf(event) == -1) {
                    this.$el = $el.on(event, '[sn-on]', $.proxy(this.handleEvent, this));
